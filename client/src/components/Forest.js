...
function Forest({currentUser}) {
  const [forestsArray, setForestsArray] = useState([]);
  const [loading, setLoading] = useState(true); // Loading state
  const navigate = useNavigate();
  const [hoveredForest, setHoveredForest] = useState(null);
    ...
    <div className="relative h-64 md:h-80 mb-8">
      ...
      <div className="absolute inset-0 flex items-center justify-center flex-col text-white px-4">
        ...
        <p className="text-lg md:text-xl max-w-2xl text-center">
          Explore the world's most beautiful Forests with Explorr
        </p>
      </div>
    </div>
    ...
    <section className="px-4 md:px-8 lg:px-16 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-2">
        ...
        <div className="bg-white px-3 py-1 rounded-full shadow-sm text-sm text-gray-600">
          {forestsArray.length} destinations
        </div>
      </div>
        ...
        {forestsArray.map((forest) => (
          ...
          >
            <div className="relative h-56 md:h-64 overflow-hidden">
              <img
                src={forest.image}
                alt={`${forest.name} Beach`}
                className={`w-full h-full object-cover transition-transform duration-700 ${
                  hoveredForest === forest.id ? "scale-110" : "scale-100"
                }`}
              />
            ...
            <div className="p-5">
              ...
              <button
                onClick={() => handleButtonClick(forest.id)}
